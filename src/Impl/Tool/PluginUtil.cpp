...
plug::Plugin* loadPlugin(const char *dll_path)
{
    ...
    if (!load_plug)
    {
        PROCESS_ERROR(ERR_FILE_OPEN, "failed get plugin by %s. Error: %s.\n", dll_path, dlerror());
    ...
    if (!plugin)
    {
        PROCESS_ERROR(ERR_FILE_OPEN, "plugin is nullptr by %s. Error: %s.\n", dll_path, dlerror());
        assert(0 && "plugin is nullptr");
    }

    if (dlclose(library) < 0)
    {
        PROCESS_ERROR(ERR_FILE_CLOSE, "failed close library by %s. Error: %s.\n", dll_path, dlerror());
