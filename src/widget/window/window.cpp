...
Window::Window(const char *path_texture, const Dot left_up):
               left_up_(left_up), width_(0), hieght_(0), background_() 
{
...
void Window::Draw(sf::RenderTarget &target) const
{

    sf::Sprite sprite;
    sprite.setTexture(background_);
    sprite.setPosition((float)left_up_.GetX(), (float)left_up_.GetY());
...
bool Window::OnMouseMoved(const int x, const int y)
{
...
bool Window::OnMousePressed(const MouseKey key)
{
...
bool Window::OnMouseReleased(const MouseKey key)
{
