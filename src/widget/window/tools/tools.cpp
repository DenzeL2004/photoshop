...
class LineWidget : public Widget
{
    public:
        ...
        void Draw               (sf::RenderTarget &targert, Container<Transform> &stack_transform)
        {
            ...
            // sf::Vector2f start   = last_trf.RollbackTransform();
            // sf::Vector2f end     = last_trf.RollbackTransform(end_pos_   - last_trf.offset);
...
void LineTool::OnConfirm (const Dot &pos, Canvas &canvas)
{
    ...
    Dot begin   = start_pos_ + real_pos;
        begin.y = canvas_size.y - begin.y;
    Dot end     = pos + real_pos;
        end.y   = canvas_size.y - end.y;
